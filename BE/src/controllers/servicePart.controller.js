...
const svc = require('../services/servicePart.service');

async function list(req, res, next) {
  try {
    const data = await svc.list({
      serviceId: req.query.serviceId ? parseInt(req.query.serviceId,10) : undefined,
      partId: req.query.partId ? parseInt(req.query.partId,10) : undefined
    });
    res.json(data);
  } catch (e) { next(e); }
}
